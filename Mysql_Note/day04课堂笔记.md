...
# 增加一个全文索引（索引名）列名
alter table 数据库.表名 add fulltext index (索引名) 列名

# explain 分析 sql 执行状况
explain select * from student;	# 非全文索引

explain select * from student where match(列名) against('字段')
```

1. 选择需要添加索引的表：确定需要天剑索引的名称。
2. 分析查询需求：分析查询需求，确定需要添加索引的列。通常，选择经常用于查询和连接的列作为索引列。
3. 选择索引类型：MySQL 支持多种索引类型，包括 b-tree 索引、哈希索引、全文索引等。根据查询需求和数据特点选择合适的索引类型。
4. 创建索引：使用 create index 语句来创建索引。语法如下：

```mysql
create [unique] index index_name on table_name(column_name);

# 其中，index_name 是索引的名称，table_name 是表名，column_name 是需要添加的列名。如果需要创建唯一索引，可以在 create index 语句中添加 unique 关键字。
```

5. 查看索引： 使用 show index 语句来查看表的索引情况。语法如下：
...
在MySQL中，创建分区函数的SQL语法如下：

```mysql
CREATE FUNCTION partition_function_name (column_name column_data_type)
RETURNS integer
BEGIN
    -- 分区函数的逻辑
    -- 根据 column_name 的值计算分区号并返回
    -- 返回的值必须是整数类型
END;

# partition_function_name 是分区函数的名称；
# column_name 是用于分区的列名；
# column_data_type 是该列的数据类型。 
```

在分区函数的逻辑中，根据 column_name 的值计算分区号，并将其返回。返回的值必须是整数类型，用于指定该行数据所属的分区。

需要注意的是，分区函数必须是确定性函数，即对于相同的输入，必须返回相同的输出。此外，分区函数还需要满足一些其他的要求，如不能使用非确定性函数，不能使用用户变量等。

创建完分区函数后，可以使用该函数来定义分区表的分区方式。具体的语法如下：

```mysql
CREATE TABLE table_name (
    ...
)
PARTITION BY RANGE (partition_function_name(column_name)) (
...
### 5.2、在 SQL Server 创建分区函数

在 SQL server 中创建分区函数的SQL语法如下：
...
# partition_function_name 是分区函数的名称；
# data_type 是用于分区的列的数据类型；
# RANGE 表示范围分区；
# [LEFT|RIGHT] 关键字用于指定边界值的位置。
# value1, value2, ... 是分区的边界值，用于将数据划分不同的分区中。
```

例如，创建一个基于整数列的范围分区函数的示例：
...
上述示例创建了一个名为 partition_func 的分区函数，使用 INT 类型的列进行分区。边界值为 100、200和300，将数据划分到小于100、100-200、200-300和大于300的四个分区中。

创建完分区函数后，可以使用该函数来定义分区方案或分区表。具体的语法如下：
...
# partition_function_name 是分区函数的名称;
# filegroup1, filegroup2, ... 是用于存储不同分区数据的文件组。
# table_name 是表名;
# column_name 是用于分区的列名。
```
